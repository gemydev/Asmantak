...
class Particle:
    ...
    def update_velocity(self,
                        pos_best_global):  # basically we need the global best of all particles, we dont have this in our class
...
class PSO:
    def __init__(self, res, costFunc, x0, bounds, num_particles, maxiter):
        ...
        for i in range(0, num_particles):
            swarm.append(
                Particle(x0))  # at first we create num_particles in the same position, random different velocities tho
...
class Method:
    ...
    def constructCoeffMatrix(
            self):  # we have 7 features for each data row so we have to initialize 7+1(free) coefficient which will later be learnt
